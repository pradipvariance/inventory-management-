        ...
        <div className="min-h-screen bg-white">
            ...
            {showQuickPOModal && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
                    <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md animate-scale-in border border-slate-200">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-bold text-slate-900 tracking-tight">Create purchase order</h2>
                            <button onClick={() => setShowQuickPOModal(false)} className="p-2 rounded-xl text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors" aria-label="Close">
                                <Plus size={20} className="rotate-45" />
                            </button>
                        </div>

                        <div className="mb-6 p-5 bg-indigo-50 rounded-xl border border-indigo-100">
                            <p className="text-xs font-semibold text-indigo-600 uppercase tracking-wider mb-1">Product</p>
                            <p className="text-lg font-bold text-slate-900">{selectedItem?.product?.name}</p>
                            <div className="flex justify-between mt-4 gap-4">
                                <div>
                                    <p className="text-[10px] font-semibold text-slate-500 uppercase">Warehouse</p>
                                    <p className="text-sm font-medium text-slate-800">{selectedItem?.warehouse?.name}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-[10px] font-semibold text-slate-500 uppercase">Current stock</p>
                                    <p className="text-sm font-bold text-rose-600">{selectedItem?.itemQuantity} units</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleCreatePO} className="space-y-5">
                            <div>
                                ...
                                <select
                                    required
                                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-200 focus:border-indigo-300 outline-none text-slate-900 transition-all"
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" min="1" required className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-200 outline-none text-slate-900" value={poForm.quantity} onChange={e => setPoForm({ ...poForm, quantity: e.target.value })} />
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Unit cost ($)</label>
                                    <input type="number" min="0" step="0.01" required className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-200 outline-none text-slate-900" value={poForm.unitCost} onChange={e => setPoForm({ ...poForm, unitCost: e.target.value })} />
                                </div>
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Est. delivery date</label>
                                <input type="date" required className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-200 outline-none text-slate-900" value={poForm.deliveryDate} onChange={e => setPoForm({ ...poForm, deliveryDate: e.target.value })} min={new Date().toISOString().split('T')[0]} />
                            </div>
                            <div className="flex gap-3 pt-2">
                                <button type="button" onClick={() => setShowQuickPOModal(false)} className="flex-1 px-4 py-3 bg-slate-100 text-slate-600 rounded-xl hover:bg-slate-200 font-semibold text-sm transition-colors">
                                    Cancel
                                </button>
                                <button type="submit" disabled={creatingPO} className="flex-1 px-4 py-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 disabled:opacity-70 font-semibold text-sm transition-colors flex items-center justify-center gap-2">
                                    {creatingPO ? <span className="h-5 w-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : 'Create order'}
                                </button>
                            </div>
                        </form>
                    </div>
